...
const JWT_SECRET = process.env.JWT_SECRET || 'clipz-ai-secret-key-2024';
...
app.post('/api/generate-clips', authenticateToken, async (req, res) => {
    try {
        ...
        // Add user ID to each clip and save to user's clips
        const userClipsData = clips.map(clip => ({
            ...clip,
            userId,
            createdAt: new Date().toISOString(),
            id: uuidv4()
        ...
        // Save clips to user's collection
        const existingClips = userClips.get(userId) || [];
        userClips.set(userId, [...existingClips, ...userClipsData]);
        
        res.json(userClipsData);
...
app.post('/api/analyze-viral-patterns', authenticateToken, async (req, res) => {
    try {
...
// API endpoint to get user's clips
app.get('/api/my-clips', authenticateToken, (req, res) => {
    try {
        const userId = req.user.userId;
...
app.delete('/api/clips/:clipId', authenticateToken, (req, res) => {
    try {
        ...
        userClips.set(userId, filteredClips);
